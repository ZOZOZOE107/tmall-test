...
function drawTapes(now: number, w: number, h: number, lms?: NormalizedLandmark[]) {
  ...
  if (!ctx) return
  if (!showGarment || garmentMode !== 'mesh' || !lms) return
...
function loop() {
  ...
  // 下面按蒙皮再画一遍会把布覆盖掉
  const flying = stepArriving(performance.now(), lms, w, h, dpr)

  for (const g of meshAll()) {
    if (flying.has(g)) continue
    if (showGarment && garmentMode === 'mesh' && g.ready && lms) {
  ...
  stepFalling(performance.now(), w, h, dpr)
  ...
  if (showGarment && garmentMode === 'rigid' && garment.ready && lms) {
  ...
  let pinchAt = hands?.length ? pinch(hands, w, h) : null
  if (pinchAt && stage.mirrored) pinchAt = { x: w - pinchAt.x, y: pinchAt.y }
  handlePinch(pinchAt, now)
