...
export interface StandMetrics {
  /** 肩中点→膝中点的 y 跨度 ÷ 人形框高，达到新版三分之二身位即为 ok */
  span: number
  /** 旧版脚底指标，保留字段以兼容调试面板；新版不再用脚踝判定 */
  foot: number
 ...
 * 张开的 y=296，左右对称所以 centerX 就是路径包围盒的中点。新版只要求
 * 肩髋膝进入有效区域，脚踝可以在画面外。距离区间用仓库内置的标准叉腰图
 * 实测校准：肩到膝约占框高 0.64；区间适当放宽，避免不同身材/机位让人明明
 * 已经进入三分之二身体却一直被判定为过近或过远。
...
/**
 * 肩到膝占人形框高度的合格区间。
 * 标准测试图实测 0.64；0.42~0.82 能覆盖常见摄像头高度和身材差异，同时仍能
 * 拦住明显站得太远/太近的人。这里直接写区间，不再用不存在的“轮廓膝线”猜目标值。
 */
const SPAN_MIN = 0.42
const SPAN_MAX = 0.82
/**
...
export class StandGuide {
  ...
  update(lms: NormalizedLandmark[] | null, dtMs: number, f: StandFrame): StandReport {
    ...
    // 新版人形框三分之二身体即可触发：肩、髋、膝齐了就能判断站位，脚踝不再硬依赖。
    // 姿势另要肘和腕（手插在腰头里时腕的置信度会偏低，所以下限和滑杆共用一份）。
    const bodyOk = !!lms && [11, 12, 23, 24, 25, 26].every((i) => vis(i) >= f.minVisibility)
    ...
    if (bodyOk) {
      ...
      const kneeMid = mid(at(25), at(26))
      const torso = len(shoulderMid, hipMid)

      // ① 远近：用肩到膝的跨度。脚还没入镜时也能进入锁定流程。
      const span = (kneeMid.y - shoulderMid.y) / f.box.h
      const foot = 0
      distance = span < SPAN_MIN ? 'far' : span > SPAN_MAX ? 'near' : 'ok'
